...
| OpenAI | entity | 특정 회사 |
| wiki-llm 패턴 | concept | 재현·적용 가능한 아키텍처 |
| Industry 4.0 | concept | 운동·담론 (고유 주체 없음) |

**모호할 때 (1분 이상 고민될 때)**: `concepts/`에 배치하고 frontmatter에 `needs-review: true`를 추가한다. Lint가 나중에 재분류 기회를 준다 — 지금 잘못 판단하는 것보다 플래그해두는 것이 낫다.
